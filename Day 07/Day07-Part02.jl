...
global Ranking = 1
global Labels = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']
global Groups = []
global Games = []
global Sum = 0
...
function GetType(array, jokers)
    if jokers != 0 
        ...
        elseif size(array)[1] == 2
            if size(filter(a -> a == 2, array))[1] != 2
                return FourOfAKind
            else
                return FullHouse
            end
...
function GetAttributes(hand)
    ...
    for i in 1:13
        if i != 13
            ...
            jokers = jokers + size(filter(s -> s == i, strengths))[1]
        end
    end

    equalCards = filter(e -> e != 0, equalCards)

    return [(GetType(equalCards, jokers)), strengths]
end
